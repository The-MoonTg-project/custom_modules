...
async def set_gemini(_, message: Message):
    if len(message.command) < 2:
        ...
        return (
            ...
            else await message.reply(usage)
        )
...
async def gemini_query(app: Client, message: Message):
    ...
    if not prompt:
        return (
            await message.edit("❌ Prompt cannot be empty.")
            if message.from_user.is_self
            else await message.reply("❌ Prompt cannot be empty.")
        )

    is_self = message.from_user and message.from_user.is_self
    wait_msg = await (message.edit if is_self else message.reply)(
        "<code>Thinking...</code>"
    )
        ...
        if message.reply_to_message:
            ...
            for attr in ["document", "audio", "video", "voice", "video_note"]:
                ...
                if media_obj:
                    ...
                    ext = (
                        Path(filename).suffix if filename else ext_map.get(attr, ".bin")
                    )
                    path = os.path.join(
                        TEMP_FILE_DIR, f"{media_obj.file_unique_id}{ext}"
                    )
            ...
            if replied.photo:
                path = os.path.join(
                    TEMP_FILE_DIR, f"{replied.photo.file_unique_id}.jpg"
                )
                await replied.download(path)
                files.append(Path(path))
