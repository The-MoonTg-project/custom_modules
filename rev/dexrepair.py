...
@Client.on_message(filters.command("dexrepair", prefix) & filters.me)
async def dexrepair_handler(_, message: Message):
    reply = message.reply_to_message
    if not reply:
        await message.edit("Please reply to a message containing a dex file.")
        return

    await message.edit("Downloading dex file...")
    dex_file = await reply.download()
    await message.edit("Repairing dex file...")
    try:
        repair_dex_file(dex_file, f"{dex_file}.repaired.dex")
        if os.path.exists(f"{dex_file}.repaired.dex"):
            await message.edit("Dex file repaired successfully.\nUploading...")
            await message.reply_document(
                document=f"{dex_file}.repaired.dex", reply_to_message_id=reply.id
            )
            await message.delete()
        else:
            await message.edit("Failed to repair dex file.")
        ...
        if os.path.exists(dex_file):
            os.remove(dex_file)
        if os.path.exists(f"{dex_file}.repaired.dex"):
            os.remove(f"{dex_file}.repaired.dex")
