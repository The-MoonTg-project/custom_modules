...
async def pasty_cmd(_, message: Message):
    ...
    if len(args) < 2 and not reply:
        await message.edit("<b>Please provide text or reply to a message</b>")
        return
    ...
    if reply:
        if reply.text:
            ...
            try:
                ...
                await message.edit(f"<b>Error reading document: {e}</b>")
                return
    else:
        text = args[-1]

    if not text:
        await message.edit("<b>No text found to paste</b>")
        return
...
modules_help["pasty"] = {
    "pasty [p/s/n/d][.ext] [text/reply]": "Paste text to various pastebin services\n"
    "p = pasty, s = spacebin, n = nekobin, d = dogbin\n"
    "Example: .pasty p.py (for python syntax highlighting)"
}
