...
def is_allowed_directory(directory):
    ...
    return any(
        absolute_directory.startswith(os.path.abspath(allowed))
...
async def list_files(_, message: Message):
    directory = (
        message.text.split(maxsplit=1)[1].strip() if len(message.command) > 1 else "."
    )

    if not os.path.isdir(directory):
        return await message.edit(
            f"<b>Invalid directory:</b>\n<code>{directory}</code>"
        )

    if not is_allowed_directory(directory):
        return await message.edit(
            f"<b>Access to this directory is restricted:</b>\n<code>{directory}</code>"
        )
    ...
    try:
        ...
        stop_time = time.time()  # Stop performance counter

        file_list = (
            "\n".join(f"{i + 1}. {f}" for i, f in enumerate(files))
            if files
            else "<b>No files found.</b>"
        )
        elapsed_time = stop_time - start_time
        text = f"<b>Files and folders in directory:</b>\n<pre>{file_list}</pre>"
        text += f"<b>Completed in {elapsed_time:.2f} seconds.</b>"
...
async def open_file(_, message: Message):
    ...
    if (
        ...
        and "Files and folders in directory:" not in message.reply_to_message.text
    ):
        return
    ...
    try:
        ...
        if command.isdigit():
            ...
            if 0 <= index < len(files):
                ...
                if os.path.isfile(file_path):
                    if os.path.getsize(file_path) > default_max_file_size:
                        return await message.reply_to_message.edit(
                            "<b>File is too large to read or upload.</b>"
                        )

                    async with aiofiles.open(file_path, "r") as f:
                        file_content = await f.read()
                    text = (
                        f"<b>Content of {files[index]}:</b>\n<pre>{file_content}</pre>"
                    )
                    ...
                    await message.reply_to_message.edit(
                        f"<b>{files[index]} is not a file or directory.</b>"
                    )
            else:
                await message.reply_to_message.edit(
                    "<b>Invalid number. Please reply with a valid number.</b>"
                )

        elif command == "back":
            parent_directory = os.path.dirname(directory)
            if parent_directory == directory or not is_allowed_directory(
                parent_directory
            ):
                return await message.reply_to_message.edit(
                    "<b>Already at the root directory or access restricted.</b>"
                )
            ...
            if len(reply_text) < 2 or not reply_text[1].isdigit():
                return await message.reply_to_message.edit(
                    "<b>Invalid command. Example: upload 1</b>"
                )

            index = int(reply_text[1]) - 1
            if 0 <= index < len(files):
                file_path = os.path.join(directory, files[index])
                if os.path.isfile(file_path) and ".env" not in file_path:
                    await message.reply_document(file_path)
                else:
                    await message.reply_to_message.edit(
                        f"<b>{files[index]} is not a file.</b>"
                    )
            else:
                await message.reply_to_message.edit(
                    "<b>Invalid number. Please reply with a valid number.</b>"
            ...
            if len(reply_text) < 3 or not reply_text[1].isdigit():
                return await message.reply_to_message.edit(
                    "<b>Invalid command. Example: rename 1 new_name.txt</b>"
                )
            ...
            if 0 <= index < len(files):
                if files[index] == ".env":
                    return await message.reply_to_message.edit(
                        "<b>Cannot rename .env file.</b>"
                    )
                old_path = os.path.join(directory, files[index])
                new_path = os.path.join(directory, new_name)
                os.rename(old_path, new_path)
                await message.reply_to_message.edit(
                    f"<b>Renamed {files[index]} to {new_name}</b>"
                )
            else:
                await message.reply_to_message.edit(
                    "<b>Invalid number. Please reply with a valid number.</b>"
            ...
            if len(reply_text) < 2 or not reply_text[1].isdigit():
                return await message.reply_to_message.edit(
                    "<b>Invalid command. Example: delete 1</b>"
                )
            ...
            if 0 <= index < len(files):
                ...
                if os.path.isfile(file_path):
                    os.remove(file_path)
                    await message.reply_to_message.edit(
                        f"<b>Deleted {files[index]}</b>"
                    )
                elif os.path.isdir(file_path):
                    os.rmdir(file_path)
                    await message.reply_to_message.edit(
                        f"<b>Deleted directory {files[index]}</b>"
                    )
                else:
                    await message.reply_to_message.edit(
                        f"<b>{files[index]} is not a file or directory.</b>"
            ...
            await message.reply_to_message.edit(
                "<b>Reply with a number to open a file or directory, 'upload' to upload a file, 'rename' to rename a file, 'delete' to delete a file, or 'back' to navigate to the parent directory.</b>"
            )
