...
from utils.scripts import generate_screenshot, no_prefix, with_reply
...
def format_youtube_results(results):
    ...
    return results, "\n\n".join(
        [
            f"{i + 1}. **[{item['title']}]({item['url']})**\nPublished by: "
            f"[{item['author']['name']}]({item['author']['url']}) - {item['views']} views - {item['duration']}"
            for i, item in enumerate(results)
        ]
    )
...
async def delete_search_data(client, chat_id, message_id):
    ...
    for key in ["google", "youtube", "movie"]:
...
async def search_music(api_url, format_function, message, query):
    ...
    search_url = f"{api_url}{query}&limit=10"
        ...
        async with session.get(search_url) as resp:
            if resp.status == 200:
                ...
                try:
                    ...
                    if err_data.get("msg"):
                        await message.edit(err_data["msg"])
                    else:
...
async def youtube_search(client: Client, message: Message):
    ...
    await message.edit("Searching...")
    search_url = f"{YOUTUBE_SEARCH_URL}{query}"
    async with aiohttp.ClientSession() as session:
        async with session.get(search_url) as resp:
            if resp.status == 200:
                data = await resp.json()
...
async def movie_search(client, message: Message):
    ...
    await message.edit("Searching...")
    search_url = f"{MOVIE_SEARCH_URL}{query}"
    async with aiohttp.ClientSession() as session:
        async with session.get(search_url) as resp:
            if resp.status == 200:
                data = await resp.json()
...
async def handle_reply(client: Client, message: Message):
    ...
    search_keys = [f"{chat_id}_google", f"{chat_id}_youtube", f"{chat_id}_movie"]
            ...
            try:
                ...
                if (
                    ...
                    if search_key.endswith("_movie"):
                        ...
                        if "image" in movie:
                            async with aiohttp.ClientSession() as session:
                                async with session.get(movie["image"]) as resp:
                                    if resp.status == 200:
