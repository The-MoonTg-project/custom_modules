...
async def gauth_cmd(_, message: Message):
    ...
    if creds:
        try:
            ...
            return await message.edit("⚠️ <b>Connected but failed to fetch info.</b>")
...
async def gtoken_cmd(_, message: Message):
    ...
    if not token_str:
        return await message.edit(
            "<b>❌ Missing Token</b>\n\n"
            "<blockquote>"
            "<b>Usage:</b> <code>.gtoken {json}</code>\n"
            "<i>Please provide the JSON string.</i>"
            "</blockquote>"
        )
...
async def gfolder_cmd(_, message: Message):
    ...
    if len(args) == 1:
        return await message.edit(
            "<b>📁 Current Folder</b>\n\n"
            "<blockquote>"
            f"<b>Path:</b> <code>{_get_folder() or 'opentg'}</code>"
            "</blockquote>"
        )
    ...
    try:
        ...
        if not creds:
            return await message.edit(
                ...
                "<blockquote>"
                "Please authenticate first using <code>.gauth</code>"
                "</blockquote>"
            )
        ...
        if not reply:
            return await message.edit(
                "❌ <b>No File</b>\n\n"
                "<blockquote>"
                "<b>Usage:</b> Reply to a file with <code>.gup</code>"
                "</blockquote>"
            )
        ...
        if not media:
            return await message.edit(
                "❌ <b>Invalid Media</b>\n\n"
                "<blockquote>"
                "Reply to a valid file (document, video, audio, photo, voice, animation)."
                "</blockquote>"
            )
        ...
        await message.edit(
            ...
            f"<b>Size:</b> {round(size/1024/1024,2)} MB\n"
            f"<b><a href='{view}'>View</a></b> | <b><a href='{direct}'>Download</a></b>"
            "</blockquote>",
            disable_web_page_preview=True
        )
...
async def glogout_cmd(_, message: Message):
    ...
    await message.edit(
        ...
        "Use <code>.gauth</code> to reconnect."
        "</blockquote>"
    )
