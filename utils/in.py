...
async def in_f(client: Client, message: Message):
    if message.reply_to_message.document:
        ...
        if os.path.exists(module):
            ...
            await message.edit(
                f"<b>Module {module_name} downloaded successfully, restarting UB!</b>"
            )
            restart()
        else:
            await message.edit(
...
async def un_f(client: Client, message: Message):
    if message.reply_to_message.document:
        ...
        if os.path.exists(f"modules/custom_modules/{module}"):
            ...
            await message.edit(
                f"<b>Module {module} removed successfully, restarting UB!</b>"
            )
            restart()
        else:
        ...
        await message.edit(
            f"<b>Usage: </b><code>{prefix}un [reply to a module name]</code>"
        )
