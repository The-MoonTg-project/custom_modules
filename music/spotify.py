...
async def spotify_download(client: Client, message: Message):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        await status_message.edit_text(
            f"<code>Downloading {download_result['data']['title']}...</code>"
        )

        try:
            thumb_path = f"{download_result['data']['title']}.jpg"
            if download_result["data"]["image"]:
                async with session.get(download_result["data"]["image"]) as resp:
                    with open(thumb_path, "wb") as thumb_file:
                        thumb_file.write(await resp.read())

            song_path = f"{download_result['data']['title']}.mp3"
            async with session.get(song_download_link) as resp:
                if "audio" not in resp.headers.get("Content-Type", ""):
                    raise ValueError("Invalid audio file")
                with open(song_path, "wb") as song_file:
                    song_file.write(await resp.read())
        except Exception:
            await status_message.edit_text(
                f"<code>Failed to download: Song isn't available.</code>"
            )
            return

    await status_message.edit_text(
        f"<code>Uploading {download_result['data']['title']}...</code>"
    )
